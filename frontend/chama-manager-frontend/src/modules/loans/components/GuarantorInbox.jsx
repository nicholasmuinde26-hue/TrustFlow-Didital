    ...
    <section className="rounded-3xl border border-amber-200 bg-amber-50 p-6 shadow-sm dark:border-amber-900 dark:bg-amber-950/30 space-y-4">
      ...
      <div className="space-y-3">
        {requests.map((r) => (
          <div
            key={r.loan_id}
            className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-amber-200/60 bg-white p-4 dark:border-amber-900/40 dark:bg-obsidian-card"
          >
            <div className="space-y-0.5">
              <div className="flex items-center gap-2">
                <span className="font-bold text-slate-900 dark:text-mist text-sm">{r.reference || "Loan Guarantee Request"}</span>
              ...
              <p className="text-xs text-slate-500 dark:text-mist-muted">Purpose: {r.purpose}</p>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 rounded-xl border border-slate-300 bg-slate-100 px-3.5 py-2 text-xs font-bold text-slate-700 hover:bg-slate-200 dark:border-obsidian-border dark:bg-obsidian-raised dark:text-mist-muted transition"
              >
