      ...
      <div className="space-y-3">
        {loans && loans.length > 0 ? (
          loans.map((loan) => (
            ...
            >
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 dark:text-mist-muted font-medium">{loan.purpose}</p>
              </div>

              <div className="flex items-center gap-4">
                <div className="text-right">
                  <span className="text-[10px] text-slate-500 uppercase font-bold block">Principal</span>
                  <span className="font-black text-slate-900 dark:text-mist text-base">{money(loan.amount)}</span>
                </div>
                <button
                  type="button"
                  onClick={(e) => {
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-10 text-center text-slate-500 dark:border-obsidian-border dark:bg-obsidian-raised/40">
            ...
            <p className="font-bold text-slate-800 dark:text-mist text-sm">No Personal Loans Found</p>
