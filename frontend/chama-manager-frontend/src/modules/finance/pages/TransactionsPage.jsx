    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 shadow-xs hover:bg-slate-50 dark:border-obsidian-border dark:bg-obsidian-card dark:text-mist transition">
            <Download size={16} className="text-slate-400" /> Export
          </button>
          <Link
            to={`/workspace/${workspaceId}/finance/record-contribution`}
            className="flex items-center gap-2 rounded-2xl bg-indigo-600 px-4 py-2.5 text-xs font-bold text-white shadow-md hover:bg-indigo-700 transition dark:bg-mint dark:text-obsidian-rail dark:hover:bg-mint-hover"
          >
      ...
      <div className="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-xs dark:border-obsidian-border dark:bg-obsidian-card space-y-4">
        {/* Category Pills */}
        <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-slate-100 dark:border-obsidian-border">
          {[
            { id: "all", label: "All" },
            ...
            <button
              ...
              className={`rounded-xl px-4 py-2 text-xs font-bold transition whitespace-nowrap ${
                activeTab === tab.id
                  ...
                  : "text-slate-600 hover:bg-slate-50 dark:text-mist-muted dark:hover:bg-obsidian-raised"
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="grid gap-3 sm:grid-cols-12 items-center">
          <div className="relative sm:col-span-6">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-slate-200 bg-slate-50/60 py-2.5 pl-10 pr-4 text-xs font-semibold text-slate-900 focus:border-indigo-600 focus:bg-white focus:outline-none dark:border-obsidian-border dark:bg-obsidian dark:text-mist dark:focus:border-mint"
            />
          </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xs dark:border-obsidian-border dark:bg-obsidian-card">
              ...
              {filteredTxList.length > 0 ? (
                filteredTxList.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50/60 dark:hover:bg-obsidian-raised/30 transition">
                    ...
                    <td className="px-6 py-4 text-slate-900 dark:text-mist font-mono font-bold">{row.reference}</td>
                    <td className="px-6 py-4 text-slate-900 dark:text-mist font-bold">{row.description}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-mist-muted">{row.account}</td>
                    <td className="px-6 py-4 text-slate-900 dark:text-mist font-bold">{row.member}</td>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 bg-white px-6 py-4 text-xs font-semibold text-slate-500 dark:border-obsidian-border dark:bg-obsidian-card">
          ...
          <div className="flex items-center gap-1.5">
            <button onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 hover:bg-slate-50 dark:border-obsidian-border dark:hover:bg-obsidian-raised">
              <ChevronLeft size={14} />
            </button>
            ...
            <button onClick={() => setCurrentPage((p) => p + 1)} className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 hover:bg-slate-50 dark:border-obsidian-border dark:hover:bg-obsidian-raised">
